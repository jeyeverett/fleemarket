...
const validateProduct = [
  ...
  check('price').trim().isCurrency().withMessage('Please enter a valid price.'),
  check('description')
    .trim()
    .isLength({ min: 50 })
    ...
    .isLength({ max: 500 })
    .withMessage(
      'The maximum length of the description should be less than 500 characters.'
    ),
];
...
const validateSignUp = [
  ...
  check(
    'password',
    'Password must be at least five characters and include only letters and numbers.'
  )
    .trim()
    .isLength({ min: 5 })
    .isAlphanumeric(),
